package access_pkg;
    ...
    class read_monitor;
        ...
        task mon_trans();
            forever begin
                string s;
                @(posedge this.rd_if.rd_clk iff (this.rd_en_next === 1'b1));
        ...
        task do_write(write_trans write_req);
            foreach(write_req.din[j]) begin
                ...
                if (write_req.wr_en[j] == 1'b1) begin
                    while (1'b1) begin
                        //@(posedge this.wr_if.wr_clk iff (this.wr_if.drv_ck.full === 1'b0));
                        ...
                        @(posedge this.wr_if.wr_clk);
                        #1step;
                        if (this.wr_if.drv_ck.wr_ack === 1'b1 && this.wr_if.drv_ck.overflow === 1'b0)
                            break;
                        //@(posedge this.wr_if.wr_clk);
                        //overflow = this.wr_if.drv_ck.overflow;
                        //s = $sformatf("if_wr_ack = %d, if_overflow = %d, wr_ack = %d, overflow = %d\n",this.wr_if.drv_ck.wr_ack,
                        //this.wr_if.drv_ck.overflow, wr_ack, overflow);
                        //rpt_pkg::rpt_msg("[write_action]", s, rpt_pkg::INFO, rpt_pkg::LOW);
                    end
                end
                else
    ...
    class write_monitor;
        ...
        task mon_trans();
            forever begin
                //@(posedge this.wr_if.wr_clk iff (this.wr_if.mon_ck.wr_ack));
                @(posedge this.wr_if.wr_clk iff (this.wr_if.mon_ck.wr_ack === 1'b1 && this.wr_if.mon_ck.overflow === 1'b0));
                //@(posedge this.wr_if.wr_clk iff (this.wr_if.wr_ack === 1'b1 && this.wr_if.overflow === 1'b0));
                this.wr_mon_data.data = this.wr_valid_data;
