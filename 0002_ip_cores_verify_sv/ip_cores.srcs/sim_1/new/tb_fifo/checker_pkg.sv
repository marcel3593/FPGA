package checker_pkg;
    ...
    class fifo_checker;
        ...
        task compare();
            ...
            if (this.read_mon_data.data == this.write_packed_data_lower.data && this.read_mon_data.id == this.write_packed_data_lower.id  
            && this.read_mon_data.data == this.write_packed_data_raw_lower.data && this.read_mon_data.id == this.write_packed_data_raw_lower.id) begin
                cmp = 1;
                rpt = rpt_pkg::INFO;
                svrt = rpt_pkg::MEDIUM;
            end
            ...
            s = $sformatf("cmp = %d, rd_id = %d, rd_data = %4b, wr_id = %d, wr_data = %4b, wr_raw_id = %d, wr_raw_data = %4b\n", cmp, this.read_mon_data.id, this.read_mon_data.data, this.write_packed_data_lower.id, this.write_packed_data_lower.data
            , this.write_packed_data_raw_lower.id, this.write_packed_data_raw_lower.data);
